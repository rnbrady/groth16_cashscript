...
import {
  ...
  armRange, jacAdd, jacToAffine, commit, compile, unlockOf, tok, realVm,
} from './_vkxmath.mjs';
...
function buildTx(forgeHandoff) {
  const sourceOutputs = [], inputs = [], outputs = [];
  // arm inputs/outputs
  for (let i = 0; i < N; i++) {
    const c = allArms[i];
    let inState = c.inState;
    // forge: tamper the consumed state of the SECOND chunk of term0 (a stitched consumer)
    if (forgeHandoff && c.label === 't0' && c.k === 1) { inState = [...c.inState]; inState[0] = (inState[0] + 1n) % P; }
    sourceOutputs.push({ lockingBytecode: c.label === 't0' ? armLock('t0', c.k) : armLock('t1', c.k), valueSatoshis: 1000n, token: tok(commit(c.inState)) });
